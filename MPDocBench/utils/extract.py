...
# img
img_pattern = r'!\[.*?\]\(.*?\)'
# figure bbox pattern: pageX_x1_y1_x2_y2.ext
figure_bbox_pattern = re.compile(r'page(\d+)_(\d+)_(\d+)_(\d+)_(\d+)\.\w+')


def extract_figure_boxes(content):
    """从 MD 内容中提取图片引用的 bounding box 信息。

    解析 ![](pageX_x1_y1_x2_y2.jpg) 格式的图片引用，
    返回每个图片的页码和归一化坐标(0-1000)。
    """
    boxes = []
    for match in re.finditer(img_pattern, content):
        img_ref = match.group()
        fig_match = figure_bbox_pattern.search(img_ref)
        if fig_match:
            page_id = int(fig_match.group(1))
            x1, y1, x2, y2 = (int(fig_match.group(i)) for i in range(2, 6))
            boxes.append({
                "page_id": page_id,
...
def md_tex_filter(content):
    ...
    # title_matches = f.finditer(content)
    # if title_matches:
