...
def match_gt2pred_simple(gt_items, pred_items, line_type, img_name):
    ...
    match_list = []
    if not norm_gt_lines and not norm_pred_lines:  # 如果两者都是空的,则返回空的匹配列表
        return match_list, None

    elif not norm_gt_lines: # not matched pred should be concatenate
        pred_idx_list = range(len(norm_pred_lines))
        match_list.append({
            'gt_idx': [""],
            'gt': "",
            'pred_idx': list(pred_idx_list),
