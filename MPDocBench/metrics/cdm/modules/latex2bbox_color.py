...
def convert_pdf2img(pdf_filename, png_filename, temp_dir=None):
    ...
    # cmd = "magick -density 200 -quality 100 %s -colorspace RGB -alpha set %s"%(pdf_filename, png_filename)
    cmd = "magick -density 200 -quality 100 %s -alpha set %s"%(pdf_filename, png_filename)
...
def latex2bbox_color(input_arg):
    ...
    with open(tex_filename, "w") as w: 
        ...
        w.write(final_latex.replace(r'\mathcolor', r'\color'))
    # print(os.path.exists(tex_filename), tex_filename)
