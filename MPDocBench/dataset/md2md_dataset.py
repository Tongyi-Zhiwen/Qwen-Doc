...
def match_gt2pred_with_timeout(gt_text, pred_text, mode, img_name, timeout=30):
    """
    带超时控制的 match_gt2pred
    超过 timeout 秒自动切换简单评估
    """
    ...
    try:
        start = time.time()
        result = match_gt2pred_quick(gt_text, pred_text, mode, img_name)
        ...
        signal.alarm(0)
        elapsed = timeout
        print(f"⚠️ [{img_name}] 超过{timeout}s，切换简单评估")
        result = match_gt2pred_simple(gt_text, pred_text, mode, img_name)
        return result
    ...
    def get_matched_elements(self, gt_folder, pred_folder):
        ...
        for sample_name in tqdm(os.listdir(gt_folder)):
            ...
            if gt_dataset['text_all']:
                ...
                plain_text_match_s = match_gt2pred_with_timeout(gt_dataset['text_all'], pred_dataset['text_all'], 'text', img_name)
            ...
            if gt_dataset.get('equation_isolated'):
                display_formula_match_s = match_gt2pred_simple(gt_dataset['equation_isolated'], pred_dataset['equation_isolated'], 'formula', img_name)
            ...
            if gt_dataset.get('latex_table') and pred_dataset.get('latex_table'): # By default the model won't randomly output both latex and html, but choose one; Note that table format in GT markdown needs to match Pred
                ...
                table_match_s = match_gt2pred_simple(gt_dataset['latex_table'], pred_dataset['latex_table'], 'latex_table', img_name)
                ...
                table_match_s = match_gt2pred_simple(gt_dataset['html_table'], pred_dataset['html_table'], 'html_table', img_name)
                table_match_s = [x for x in table_match_s if x['gt_idx'] != [""]]  # Remove extra preds
