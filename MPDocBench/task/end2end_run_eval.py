...
from metrics.show_result import show_result, get_full_labels_results, get_page_split, get_figure_label_results
    ...
    def __init__(self, dataset, metrics_list, page_info_path, save_name):
        ...
        for element in metrics_list.keys():
            ...
            for metric in metrics_list[element]['metric']:
                ...
                samples, result_s = metric_val(samples).evaluate(group_info, f"{save_name}/{element}")
                if result_s:
            ...
            # 对 figure 类型，按 label 分组计算召回率
            if element == 'figure':
                figure_label_result = get_figure_label_results(saved_samples)
                result_all[element]['figure_label'] = figure_label_result

            try:

                with open(f'./result/{save_name}/{element}_result.json', 'w', encoding='utf-8') as f:
        ...
        with open(f'./result/{save_name}/metric_result.json', 'w', encoding='utf-8') as f:
