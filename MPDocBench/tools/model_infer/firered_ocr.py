...
# CUDA_VISIBLE_DEVICES=0,1,2,3 vllm serve /nas-mmu/zhoubb/download_models/FireRed-OCR --max-model-len 32768 --trust-remote-code --served-model-name firered-ocr --tensor-parallel-size 4 --port 8118 &
# CUDA_VISIBLE_DEVICES=4,5,6,7 vllm serve /nas-mmu/zhoubb/download_models/FireRed-OCR --max-model-len 32768 --trust-remote-code --served-model-name firered-ocr --tensor-parallel-size 4 --port 8119
...
QUERY_PROMPT = r'''You are an AI assistant specialized in converting document images to structured Markdown. Please follow these instructions for the conversion:
            ...
            - For example, a figure at normalized coordinates (459, 168) to (526, 230) must be output as: `![](459_168_526_230.jpg)`.
