...
from chandra.output import parse_markdown, parse_html, parse_chunks, extract_images, extract_images_bbox
...
class InferenceManager:
    ...
    def generate(
        ...
        image_indics = kwargs.pop("image_indics", list(range(len(batch))))
        ...
        output = []
        for img_idx, result, input_item in zip(image_indics, results, batch):
            ...
            output.append(
                BatchOutputItem(
                    ...
                    bboxs = extract_images_bbox(result.raw, chunks, input_item.image),
                    error=result.error,
                )
            )
        return output
