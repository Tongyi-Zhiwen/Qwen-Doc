...
def normalize_bbox_to_1000(bbox, width, height):
    return [
        int(bbox[0] / width * 1000),
        int(bbox[1] / height * 1000),
        min(int(bbox[2] / width * 1000), 1000),
        min(int(bbox[3] / height * 1000), 1000),
    ]

def extract_images_bbox(html: str, chunks: dict, image: Image.Image):
    ...
    for idx, chunk in enumerate(chunks):
        ...
        if chunk["label"] in ["Image", "Figure"]:
            ...
            try:
                ...
                continue
            img_name = get_image_name(html, div_idx)
            image_bboxs[img_name] = normalize_bbox_to_1000(bbox, width, height)
    return image_bboxs
